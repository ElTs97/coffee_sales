...
def plot_coffee_distribution(df):
    ...
    plt.pie(coffee_name_perc, labels=coffee_name_perc.index, autopct='%1.1f%%', startangle=140, colors=plt.cm.Paired(range(len(coffee_name_perc))))  
    plt.title('Distribution of Coffee Name', fontsize=16, weight='bold')
    plt.axis('equal')
...
# Display settings
pd.set_option('display.float_format', '{:,.0f}'.format)  # Remove scientific notation
pd.set_option('display.max_columns', None)  # Show all columns
pd.set_option('display.width', 1000)  # Set the width of the display

# Printing the sorted pivoted table
print(pivoted_sales)
...
# Q6: What is the distribution of payment types used by customers (count)
def plot_cash_type_distribution(df):
    ...
    plt.pie(cash_type_perc, labels=cash_type_perc.index, autopct='%1.0f%%', startangle=140,
            colors=plt.get_cmap('tab20').colors, shadow=False, wedgeprops=dict(width=0.3))
    plt.title('Distribution of Cash Type (COUNT)', fontsize=18, weight='bold', color='navy')
    plt.show()
...
# Q7: What is the total revenue percentage generated by each payment type
def plot_revenue_distribution(df):
    money_cash_type = df.groupby('cash_type')['money'].sum()

    def func(pct, allvalues):
        absolute = int(pct / 100. * sum(allvalues))
        return f'{absolute:,} ({int(pct)}%)'

    plt.figure(figsize=(10, 8))
    plt.pie(money_cash_type, labels=money_cash_type.index, autopct=lambda pct: func(pct, money_cash_type),
            startangle=140, colors=plt.get_cmap('tab20').colors, shadow=False, wedgeprops=dict(width=0.3))
    plt.title('Distribution of Cash Type (REVENUE)', fontsize=18, weight='bold', color='navy')
    plt.show()
...
# Q8: What is the monthly revenue by payment type
...
# Q10: What is the frequency of each coffee type per frequent customer
...
# Q11: What is the monthly spending per frequent customer
